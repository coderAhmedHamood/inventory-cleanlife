...
import { Plus, Edit, CheckCircle, AlertCircle, XCircle } from 'lucide-react'

export default function ScrapPage() {
  const { scrapOrders, locations, products, addScrapOrder, updateScrapOrder, addStockMove, updateStockMove } = useStore()
  ...
  const scrapLocation = locations.find((l) => l.type === 'scrap')
  ...
  const handleAddMove = () => {
    if (currentMove.productId && currentMove.quantity > 0 && formData.locationId) {
      ...
      setCurrentMove({ productId: '', quantity: 0 })
    }
  }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const orderId = selectedOrder?.id || `scrap-${Date.now()}`
    if (selectedOrder) {
      updateScrapOrder(orderId, { ...formData, moves: moveIds })
    } else {
      addScrapOrder({
        id: orderId,
        ...formData,
    ...
    setFormData({ reference: '', locationId: '', date: new Date().toISOString().split('T')[0], reason: '', moves: [] })
  }
  ...
  const handleConfirm = (order: any) => {
    ...
    updateScrapOrder(order.id, { state: 'done' })
  }
    ...
    <Layout>
      <div className="p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">الهالك</h1>
            <p className="text-gray-600 mt-1">إدارة المنتجات التالفة والهالك</p>
          </div>
          <button
            onClick={() => {
              setSelectedOrder(null)
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
              ...
              {scrapOrders.map((order) => {
                ...
                return (
                  <tr key={order.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-500">{order.reason || '-'}</td>
          ...
          {scrapOrders.length === 0 && (
            <div className="text-center py-12">
              <AlertCircle className="w-12 h-12 text-gray-400 mx-auto mb-4" />
              <p className="text-gray-500">لا توجد أوامر هالك</p>
            </div>
          )}
        </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
              <h2 className="text-2xl font-bold mb-4">
                {selectedOrder ? 'تعديل هالك' : 'إضافة هالك'}
              </h2>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {formData.locationId && (
                  <div className="border-t pt-4">
                    <h3 className="text-lg font-semibold mb-3">إضافة منتج للهالك</h3>
                        ...
                        return (
                          <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                            <div>
                              ...
                              <p className="text-sm text-gray-500">الكمية: {move.quantity}</p>
                            </div>
                            <button
                              type="button"
